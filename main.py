...
class Label:
    def __init__(self, text, x, y, font_size=30):
        self.font = pygame.font.Font("fonts/Sen-Bold.ttf", font_size) #load font
        self.text = text # save text
        self.text_surf = self.font.render(text, True, BLACK) #render text
        self.text_rect = self.text_surf.get_rect()           # get rect from text to change its position
        self.text_rect.center = (x, y)                       # changing text position to (x, y)
